...
public class CourseSchedule207 {
    ...
    public static void main(String[] args) {
        ...
        //System.out.println(canFinish(20, new int[][]{{1,0},{0,1}}));
    ...
    public static boolean canFinish(int numCourses, int[][] prerequisites) {
        ...
        List<Integer>[] post = new List[numCourses];
        int[] indegree = new int[numCourses];
        for(int[] p : prerequisites) {
            if(post[p[1]]==null){
                post[p[1]]=new ArrayList<>();
            }
        ...
        class course{
            ...
            ArrayList<course> ante=new ArrayList<>();
            ...
            public void addoPost(course course){
                this.post.add(course);
            }

            public void addoAnte(course course){
                this.ante.add(course);
                anteCount++;
            }

            public boolean contineoAnte(course course){
                return this.ante.contains(course);
            }

            public void removeoAnte(course course){
                if(!this.ante.contains(course)){
                    return;
                }

                this.ante.remove(course);
                anteCount--;
            }
        ...
        for(int[] set:prerequisites){
            ...
            if(!map.containsKey(set[1])){
                map.put(set[1],new course(set[1]));
            }

            course current=map.get(set[1]);

            if(!map.containsKey(set[0])){
                map.put(set[0],new course(set[0]));
            }

            course pointTo=map.get(set[0]);

            current.addoPost(pointTo);
            pointTo.addoAnte(current);
        ...
        bigloop: while(true) {
            Iterator<Map.Entry<Integer, course>> iterator = map.entrySet().iterator();
            while (iterator.hasNext()) {

                course entry = iterator.next().getValue();
                if (entry.anteCount == 0) {

                    for (HashMap.Entry<Integer, course> set : map.entrySet()) {

                        if (set.getValue().contineoAnte(entry)) {
                            set.getValue().removeoAnte(entry);
                        }
     ...
     *
     * My opinion:
     * Use class and hashmap, eliminate the 0 entry point until no points left
     *
     * His opinion:
     * Use Arraylist and Array of ArrayList, use a queue to eliminate the 0 entry point until no points left
     *
     * Hint:
     * 1. class can be quite time-consuming and space consuming, try using of combination of Array and ArrayList
     * 2. Queue can be used to place the infinite loop
     * */
